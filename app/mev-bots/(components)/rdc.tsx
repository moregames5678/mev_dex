...
import { NoActiveBots } from '@/app/mev-bots/(components)/no-active-bots';
import { UserTransaction } from '@/app/dashboard/(components)';
...
export const RDC = ({ blockUser = false }: props) => {
  ...
  const [userBots, setUserBots] = useState(true);
  ...
  return (
    <>
      <BoxWrapper classNameWrapper="mt-3">
        ...
        {userBots && (<div className={cn('mt-[30px] flex gap-5', { hidden: blockUser })}>
          <BoxWrapper
            variant="inside"
          ...
          >
            ...
            {tabValue.label === 'MevDex RDC' && (
              <>
                ...
                <div className="mt-7 flex items-center gap-2.5 text-sm text-green">
                  INVESTED / ON BALANCE <Solana className="ml-auto h-4 w-4" />{' '}
                  <span className="text-magenta">1.1</span>
                  <span className="text-secondary">($170)</span>
                </div>
                <div className="mt-5 flex items-center gap-2.5 text-sm text-yellow-400">
                  PENDING NEXT EPOCH <Solana className="ml-auto h-4 w-4" />{' '}
                  <span className="text-white">1.1</span>
                  <span className="text-secondary">($170)</span>
                </div>
                <div className="mt-5 flex items-center gap-2.5 text-sm text-secondary">
                  GROSS PROFIT EARNED <Solana className="ml-auto h-4 w-4" />{' '}
                  <span className="text-white">1.1</span>
                  <span className="text-secondary">($170)</span>
                </div>
              </>
            ...
            <div className="bottom-5 right-5 mt-7 grid grid-cols-2 gap-5 text-sm w-full">
                ...
                <div className="min-w-[489px]">
                  ...
                  <div className="mt-4 flex">
                    ...
                    >
                      ...
                      <div
                        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-xs text-blue">
                        <div>45.6</div>
                        <div>MS</div>
                      </div>
                    </BoxWrapper>
      ...
      {!userBots && <NoActiveBots className="mt-6" />}
      {userBots && (<div className="mt-6"><UserTransaction /></div>)}
    </>
  );
};
